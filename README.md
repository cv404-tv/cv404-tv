...
`/events/jev` 为第二期活动专题，提供 Jev 模型说明、预设决策演示、项目方向、拟定一日流程与评审建议，以及邮箱登录后的报名意向提交。活动时间为 2026-09-27 10:00–20:00，地点为云谷中心 B2-3F 活动区，免费报名、限 30 名；比赛提供模型使用额度，具体领取规则待通知。最新海报在 `public/assets/jev-poster.png`，与 `outputs/jev-rocket-covers/poster-02.png` 内容一致；海报列出云谷404、杭州AI工坊、杭州黑客与画家和 YOUR SPACE。`/events/jev/slides` 展示第二期活动演示稿。每个账户每期仅保存一份申请，可返回本页查看回执与审核反馈；`/admin` 提供“黑客松报名”列表与审核功能。
