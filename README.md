...
基于 **Next.js App Router + React** 的云谷404官网。首页是「404 信号搜寻」游戏；`/events` 是按状态分组的活动目录，活动介绍与报名留在各自的详情页。游戏保留 CRT 电视外观，活动目录使用独立的列表布局。
...
`/` 是沿用 CRT 电视外壳的调频小游戏，旧 `/signal` 仍可访问。页头和共享页脚提供活动入口，支持中英文与日夜主题。
...
- 本期三个故事分别进入第二期活动、第一期回顾和创作指南；活动目录不使用频道 hash。
...
其他页面右上角「锐评一下」进入 `/tier`，锐评页面自身隐藏此入口。
...
- 翻译覆盖游戏、活动目录与详情、指南及 404 页面。品牌名称随语言切换：中文为“云谷404”，英文为“Cloud Valley 404”；海报和原始指南是中文历史材料，英文页面已标注。
- 切换语言或主题不会重置游戏进度。

## 代码组织

- `app/`：Next.js 布局、游戏首页、活动页、指南和 404 路由。
- `components/events-list.jsx`、`lib/events.js`：活动目录布局与双语活动记录。
...
- `src/television.css`、`src/tv-effects.css`：游戏中 CRT 机身的材质与玻璃效果。
- `public/assets/walnut-grain.svg`：本地程序生成木纹，无外部贴图请求。
- 游戏旋钮支持指针操作与键盘微调；声音默认关闭，可手动开启。
...
品牌色为米白 `#F2EBDD`、墨黑 `#22272A`、红色 `#C83832`。活动目录沿用这组颜色；系统减少动态效果时关闭可选动画。

`node_modules/`、`.next/`、`out/`、`.wrangler/`、`output/` 和本地凭据不提交。官网当前未接入 shadcn/ui；迁移后的 React 结构可继续接入所需组件。

## 第二期 Jev 黑客松

`/events/jev` 为第二期活动专题，提供 Jev 模型说明、预设决策演示、项目方向、拟定一日流程与评审建议，以及邮箱登录后的报名意向提交。活动时间为 2026-09-27 10:00–20:00，地点为云谷中心 B2-3F 活动区，免费报名、限 30 名；比赛提供模型使用额度，具体领取规则待通知。最新海报在 `public/assets/jev-poster.png`；制作源文件和其他海报版本保存在独立工作文件夹的 `网站素材与制作/`，不参与网站构建。海报列出云谷404、杭州AI工坊、杭州黑客与画家和 YOUR SPACE。合作方标识保留在 `public/assets/jev-partner-*`，供活动页和后续活动复用。`/events/jev/slides` 展示第二期活动演示稿。每个账户每期仅保存一份申请，可返回本页查看回执与审核反馈；`/admin` 提供“黑客松报名”列表与审核功能。
