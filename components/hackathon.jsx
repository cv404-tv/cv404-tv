  ...
  return <div className="jev-site"><a className="skip-link" href="#jev-main">{locale === 'zh' ? '跳转到内容' : 'Skip to content'}</a><Header />
    <main id="jev-main" className="jev-page">
      ...
      <section className="jev-hero">
        <div className="jev-hero-copy"><span className="jev-badge"><i />{t.badge}</span><h1>{t.title[0]}<br /><em>{t.title[1]}</em></h1><p>{t.intro}</p><div className="jev-actions"><a className="jev-button" href="#apply">{t.apply}<span>↗</span></a><a className="jev-text-link" href="#about">{t.learn} ↓</a><Link className="jev-text-link" href="/events/jev/slides">{locale === 'zh' ? '观看活动 PPT' : 'View event slides'} ↗</Link><a className="jev-text-link" href="/assets/jev-poster.png" target="_blank" rel="noopener noreferrer">{t.poster} ↗</a></div></div>
      ...
      <section className="jev-logistics" aria-label={t.label}>{[[t.date,t.dateValue],[t.place,t.placeValue],[t.capacity,t.capacityValue]].map(([label,value]) => <div key={label}><span>{label}</span><strong>{value}</strong></div>)}<p>{t.pendingInfo}</p></section>
      ...
      <section className="jev-section jev-judging"><div className="jev-section-heading"><span className="jev-kicker">04 / THE REVIEW</span><h2>{t.judgingTitle}</h2><p>{t.judgingIntro}</p></div><div className="jev-judging-grid">{t.judgingCriteria.map(([score,label]) => <div key={label}><strong>{score}</strong><span>{label}</span></div>)}</div></section>
      <section id="apply" className="jev-registration"><div><span className="jev-kicker">05 / JOIN THE NEXT CHAPTER</span><h2>{t.registration}<br /><em>{t.registrationAccent}</em></h2><p>{t.registrationBody}</p><div className="jev-registration-note"><span className="jev-badge"><i />{t.badge}</span><p>{t.pendingInfo}</p></div></div><div className="jev-form-card">{!auth.ready ? <p role="status">{t.loading}</p> : !auth.user ? <><span className="jev-form-index">APPLICATION / 002</span><h3>{t.apply}</h3><p>{t.loginHint}</p><button className="jev-button" onClick={auth.show}>{t.login} ↗</button></> : <Application key={auth.user.id} user={auth.user} t={t} track={track} showLogin={auth.show} />}</div></section>
