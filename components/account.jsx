...
function AccountContent({ auth, t, locale }) {
  ...
  return <>
    ...
    <div className="management-columns">
      ...
      <section className="token-panel"><h2>{t.quickLinks}</h2><div className="management-links">{[['/events/jev#apply', 'hackathon'], ['/tokens', 'tokens'], ['/tier', 'tier'], ['/guide', 'guide']].map(([href, name]) => <Link key={name} href={href}><strong>{t[name]} <span aria-hidden="true">↗</span></strong><span>{t[`${name}Hint`]}</span></Link>)}{auth.user.isAdmin && <Link href="/admin"><strong>{t.adminTitle} ↗</strong><span>{t.adminIntro}</span></Link>}</div></section>
    </div>
    <PreferenceSettings />
